...
def gather_sequence_info(sequence_dir, detections):
    image_filenames = {
        int(os.path.splitext(f)[0]): os.path.join(sequence_dir, f)
        for f in os.listdir(sequence_dir)
    }
    feature_dim = detections[:, 8:].shape[1]

    if len(image_filenames) > 0:
        image = cv2.imread(
            next(iter(image_filenames.values())), cv2.IMREAD_GRAYSCALE)
...
run(args.images, output, args.output_file, confidence, nms_thesh,
